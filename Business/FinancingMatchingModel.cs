    ...
    public class FinancingMatchingModel : BaseModel<FinancingMatching>
    {
        ...
        public IQueryable<FinancingMatching> GetMatching_BYAccountID(int AccountID)
        {
            var query = from a in Context.Financings
                        ...
                        where a.WorkFlowManagerID != 4 && a.Amount <= b.MaxQuota && a.Owner_A_ID == AccountID && a.Status == 0 && a.AuditStatus==1 && (a.MinTimeLimit + a.MaxTimeLimit ?? 0) <= b.MaxMonth && a.AuditStatus == 1
                        orderby a.ID
        ...
        {
            var query = from a in Context.Financings
                        where a.WorkFlowManagerID == 4 && a.Owner_A_ID == AccountID && a.Status == 0 && a.AuditStatus == 1 
                        orderby a.ID
        ...
        public IQueryable<FinancingMatching> GetMatching()
        {
            var query = from a in Context.Financings
                        from b in Context.MechanismProducts
                        where a.WorkFlowManagerID != 4 && a.Amount <= b.MaxQuota && a.Status == 0 && a.AuditStatus == 1 && (a.MinTimeLimit + a.MaxTimeLimit ?? 0) <= b.MaxMonth && a.AuditStatus == 1
                        orderby a.ID
                        select new FinancingMatching { FID = a.ID, FName = a.Name, MID = b.ID, MName = b.Name };
            return query;
        }
